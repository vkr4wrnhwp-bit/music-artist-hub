...
def test_the_release_door_comes_back_to_the_room_with_the_done_line():
    ...
    # Merged 2026-09-23 with the Fans, Marketing and Command Center fixes of
    # the same save: it lands on the new release's edit page (where it gets
    # its destinations), carrying the way back with from= folded in, so the
    # edit page's "Back to the releases room" is the done line.
    loc = r.headers["Location"]
    assert loc.startswith("/links/") and "/edit?returnTo=" in loc, loc
    edit = c.get(loc).get_data(as_text=True)
    back = edit.split('id="sb-room-back"', 1)[0].rsplit('href="', 1)[1].split('"', 1)[0].replace("&amp;", "&")
    assert back == "/room/releases?from=releases-zero-state", back
    page = c.get(back).get_data(as_text=True)
