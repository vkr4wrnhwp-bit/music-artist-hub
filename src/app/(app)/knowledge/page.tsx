...
import { SUBJECT_LABEL, type DisagreementSubject } from "@/lib/disagreement";
import { TopBar } from "@/components/nav";
import { EmptyState, Notice, Panel, SectionHeading, StatusChip, type Tone } from "@/components/ui";
...
export default async function KnowledgePage() {
  ...
  const [knowledge, disagreements] = await Promise.all([
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6">
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <Notice tone="review" title="Promotion to knowledge is a manual step">
            A disagreement does not become shop knowledge on its own. One person's objection, recorded once, is not yet
            something CANVAS should plan from — it becomes that when somebody decides it has earned it. The promotion
            path exists in the data model and in <span className="font-mono">promoteToShopKnowledge()</span>; the review
            queue that drives it is not built yet, so observations currently arrive from seeded and job history only.
          </Notice>
        </div>
      </main>
    </>
