"""REACH web layer — a Flask blueprint mounted inside the host application.

The host app owns the shell, the design system and the routing conventions.
REACH adds one blueprint under ``/reach`` and reuses ``templates/base.html``.
"""
...
def bootstrap():
    """Idempotent start-up work: tenant, principal, policies, catalog mirror."""
    rbac.ensure_default_tenant()
    policy.seed_policies()
    catalog.sync_from_host()
...
def new_campaign():
    ...
    return render_template(
        ...
        audio_state=royalty_bridge.audio_analysis_state(),
