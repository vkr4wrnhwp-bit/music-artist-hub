...
(function () {
  ...
  function renderCues() {
    ...
    var sorted = show.cues.slice().sort(function (a, b) { return a.t - b.t; });
    if (!sorted.length) {
      cuesWrap.innerHTML = '<p class="text-[11px] text-gray-300">No cues yet. ' +
    ...
    sorted.forEach(function (c) {
      ...
      fadeWrap.className = "flex items-center gap-1 text-[10px] text-gray-300";
      fadeWrap.textContent = "fade";
      fadeWrap.appendChild(inp("number", c.fade, "w-16 rounded border border-white/15 bg-black/50 px-2 py-1 font-mono text-xs text-gray-200",
        function (e) { c.fade = parseFloat(e.target.value) || 0; }, {step: "0.1", min: "0"}));
      row.appendChild(fadeWrap);
      var del = document.createElement("button");
      del.textContent = "×";
      del.className = "ml-auto h-7 w-7 rounded border border-white/15 text-sm text-gray-300 hover:border-red-500/40 hover:text-red-400";
