...
WORKHOLDING_LOAD_DIRECTION_REVIEW is now built. The old entry said the check needed "a cutting-force vector and a jaw axis, and CANVAS records neither". Half of that stopped being true when `Setup.jawAxis` was added for the fixture-collision work; the other half turned out not to be needed, because the question a machinist asks is which way, not how many pounds. `holding-margin.ts` answers magnitude and says DEVELOPMENT ANALYSIS while it does. Direction is stated exactly by the program itself: every cutting segment has a feed vector in its own coordinates.

The rule it encodes: a cut pushing the part along the jaw axis drives it into a jaw, and the jaw body reacts that directly. A cut pushing across the jaw axis slides the part along the jaw faces with nothing but friction resisting — which is how a part walks out of a vise while every clamping number looks fine. `Setup.hasPositiveStop` reacts exactly that, so it is checked first and the finding is withheld (reported through `checksSkipped`, not silently) when a stop is recorded. Share is weighted by cutting distance rather than counted per segment, so a long slot across the jaws outweighs a short one, and a cut at 30 degrees contributes partly to each rather than rounding to whichever way it leans. Plunges are excluded: they push the part down onto the parallels. Where no jaw axis is recorded the check still declares itself skipped and names the missing datum rather than printing an invented direction.

`src/lib/nc/load-direction.ts` (new), `src/lib/nc/analyze.ts` (finding block, `workholding` context), `src/app/api/parts/[id]/nc-analyze/route.ts` (passes the primary setup), `tests/engines/load-direction.test.ts` (17 tests).

Noticed while wiring it and left alone: `Setup.loadDirection` is a nullable String column that no code in `src/` reads or writes. It predates this work and is not what the check uses — the direction here is derived from the program's own coordinates, not stored. It is recorded here so the next reader does not take it for a datum the analysis depends on.
