...
- Files: static/css/room-kit.css (.rk-pl*); PLATE in fan_room.py, stage_room.py, analytics_room.py, publishing_room.py, releases_room.py, marketing_room.py; static/img/{fans,stage,analytics,publishing,releases,marketing}-plate.webp
...
- Files: app.py room_screen() -> app.py _publishing_room(), catalog_new(); templates/room_publishing_error.html; templates/partials/cc_rack.html + static/css/command-zero.css + static/img/command-plate.webp (the page from zero); publishing_room.py new_account(), done_line(), zero_page(), STATES, PUBLISHING_BUCKETS, collecting_titles(), state_of(), states(), uncollected(), headline(), splits(), build(); templates/room_publishing.html; static/css/publishing-room.css; rooms.py ROOMS["publishing"]; engines artist_os.py (mlc_evidence, lockbox_report), rights_conflicts.py, royalty_types.py; stores db.py (os_tracks, track_mlc_checks, statement_rows)
...
- Files: app.py room_screen() -> app.py _studio_room(); studio_room.py STEPS, clock(), decibels(), source_line(), title_of(), analyser(), needle(), covers(), path(), zero_page(), done_line(), build(); templates/room_studio.html; templates/partials/cc_rack.html (the page from zero); static/css/studio-room.css; static/css/command-zero.css (the plate's rules); static/img/studio-bus-plate.webp; static/img/command-plate.webp; rooms.py ROOMS["studio"]; stores db.py (track_analysis, os_tracks), release_ready_store.py, artwork_config.py
...
- THE PLATE (2026-09-22): a photographed SHOW CONTROL desk, static/img/stage-plate.webp (1859x846), fractions in stage_room.PLATE. Three upper windows carry cues, channels and the passport version; the wide lower one carries the CUE LIST, which appears on no other part of this room. Deliberately NOT a picture of the plot: the plot has its own panel below and that panel is the real designer, so a second drawing of it would be the screen folding onto itself.
- Routes: GET /room/stage (dispatched from /room/<room_key>)
- Files: app.py room_screen() -> app.py _stage_room(); stage_room.py STEPS, timecode(), rig(), address_of(), fixtures(), fixture_groups(), cue_name(), cues(), span(), plot(), figures(), path(), new_account(), done_line(), zero_page(), build(); templates/room_stage_error.html; templates/partials/cc_rack.html + static/css/command-zero.css + static/img/command-plate.webp (the page from zero); tour_os.py create() (one_off: name, returnTo); templates/room_stage.html, templates/_sg_inputs.html; static/css/stage-room.css; rooms.py ROOMS["stage"]; engines stage_plot_catalog.py, passport_store.py; stores db.py (light_shows, stage_plots, stage_plot_images), passport_store.py (passports, passport_versions)
...
- THE PLATE (2026-09-22): the screen opens on a photographed Audience Monitor, static/img/fans-plate.webp (1860x846). Four windows, each positioned as a fraction of the plate held in fan_room.PLATE and measured off the file with PIL: the constellation at 6.67/18.56/59.30/62.29, then ON FILE, REACHABLE and NEW down the right at 71.02% wide 22.20%. The figures strip and the map panel that used to hold them are gone. Two things moved with the map rather than going out with it: the city names, now SVG <text> inside the viewBox because the window is not the map's aspect and a label placed as a percentage of the WINDOW sits off its dot; and the Live/Sample mark, which is what stops a demo account's generated audience reading as real. REACHABLE is now a stricter figure than it was: fan_segments.contactable() required only an unsuppressed row and now requires an email address too, because you cannot email somebody who has no address.
...
- THE PLATE (2026-09-22): a photographed BROADCAST unit, static/img/marketing-plate.webp (1859x846), fractions in marketing_room.PLATE. A tall window carries THE STORY - the newest announcement on file, headline falling back to title - beside a 2x2 grid of the press funnel. TWO LABELS THE AUDIT CHANGED BEFORE THE PLATE WAS RENDERED: "SENT HERE" rather than "pitched", because press_pitches.sent_at is written only by the platform's own send loop, so a pitch posted from the artist's own inbox never lands in it; and "VIEWS" rather than "opens", because press_store.mark_opened fires when a journalist LOADS the announcement page and a window marked OPENS would be read as an email open every time. The band above the plate is smart-link data and does not overlap it; the gold rail below covers the same four concepts in words, which is an open question for the owner.
...
- STILL TO COME on this page: the owner's photographed plate for the rack (static/img/command-plate.webp - the drawn CSS rack stands in until then, same markup); the gradual transition rules after the first asset / link / fan / statement (Pass 5); onboarding analytics events; the sidebar regroup, which the owner's own spec holds until the state machinery has settled.
