...
- Because: a fan is known on a smart link when the request carries their signed ?f= (fan_mail.fan_token, itsdangerous salt "fan-link", no expiry): the listen link in each release-day email app._send_release_emails sends them, which the page passes on to its own service buttons (templates/link_campaign.html, a[data-go]), and the token POST /l/<slug>/subscribe hands back after a sign-up, which the page script sets on those buttons. app._link_fan() accepts it only for a fan whose user_id is the campaign's owner. A known fan's page view is recorded with their fan_id and adds one to total_visits unless they already viewed that campaign within fan_mail.VISIT_WINDOW_MINUTES (30), so a reload is a view but not a second visit; a /go/ press adds one to total_clicks. Both go through app._credit_fan(), which re-scores intent (visits 2 points each up to 10, clicks 5 each up to 20). Anonymous views and clicks, the owner's draft preview, a token for another account's fan and a forged token credit nobody. The CRM table shows both columns under a line saying what they count; the CSV reads the same two fields. Nothing is stored on the fan's device: the token rides in the link, and the page script strips it from the address bar so a copied URL is the plain page. Until 2026-09-23 nothing wrote either counter and the CRM table had dropped both columns.
...
- Because: Every figure is real — fan_room.build() reads the same rows the Audience screen reads (links_store.list_fans + fan_audience.for_account), the tile statuses are true counts (fan-crm reads audience total, fan-club reads active club_members, marketplace counts open collab_requests), and the "Get their emails" move hands over a real CSV. The "fan lifecycle" rail counts each stage from the account's own records (fan_room.lifecycle(), since 2026-09-23; until then it was five fixed lines with nothing counted): Discover is the smart link visits the Audience funnel reads (ml_events page_view, views not people), Capture the fans on file, Know those with a name, city or country, Activate those who pressed a service button or pre-saved (fan_audience._clicked), Belong the active Fan Club members, or "No Fan Club yet". A stage with nothing counted says so in words, never 0 (tests/test_fan_room_lifecycle.py). The "Find N missing emails" move can never appear (no write path produces an empty-email fan).
