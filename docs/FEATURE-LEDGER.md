...
- STANDBY (owner, 2026-09-22: "animating these things and making them explain about what the room is doing is a better move than filling it in with data that, if someone hasn't uploaded anything yet, is blank"). When a room has measured NOTHING, its windows carry what will fill them instead of a row of "Not measured", and the bar warms through in sequence (`.rk-pl-win.is-standby::before`, a gold glow staggered by `--n`, stilled under `prefers-reduced-motion` and dropped entirely under 560px). The big window carries the room's job, NAMES ITS FEATURES, and holds one gold door. THE RULE: standby is WORDS ONLY — never a figure, not even an example one, because a demonstration number sitting where the artist's own will appear is the defect this product refuses — and nothing in it repeats what the plate silkscreens, since the photograph already prints those names in metal. Each room decides `idle` from its own tables (fans: no fan on file; stage: no show and nothing patched; analytics: no reading measured; business: no statement; publishing: no track and no registry answer; releases: no release chosen; marketing: nothing written, sent, opened or logged). A genuine measured 0 is NOT idle: it is a measurement, and it prints.
