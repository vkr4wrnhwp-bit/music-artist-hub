...
The name, tagline and accent colour a reseller's artists see instead of Street Banker's.

- Because: Name, tagline and accent save and render — the route validates the accent with brand_contrast.check_accent against the real token sheet (tools/tailwind-input.css) before pstore.set_branding writes it, and the save is audited. Probe: saving returned 200 and branding() read back {'name': 'Foxglove Music', 'tagline': 'tag', 'accent': '#ffcc55'}. The logo is not settable: partners.logo_path exists and partner_store.branding returns it, base.html and auth_base.html render brand.logo when present, but no route or form writes it — templates/partner/branding.html has only display_name, tagline and accent inputs, and set_branding's logo_path argument has no caller.
- Routes: GET+POST /partner/branding
- Files: partner_os.py:130 (branding), 111 (_brand_surfaces), partner_store.py set_branding/branding, brand_contrast.py check_accent, templates/partner/branding.html
- Access: A seat holding "branding_edit" — owner and admin only. No seat = 404. Team seats blocked.
