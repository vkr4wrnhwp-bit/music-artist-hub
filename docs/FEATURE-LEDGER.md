...
- NO FIGURE FROM THE MISSING-MONEY QUEUE: artist_os._LANE_SHARE applied six hardcoded coefficients to the whole-account total once per track, so its estimate could exceed everything the catalogue has earned. Since 2026-09-23 each figure is a share of one track's own rows (see "Money queue"), but the tile still carries a count and no money: the queue prints no total.
...
- AUDIT FIXES (2026-09-23, audit business-1..18): the done line is reachable through the real form - POST /statements from the door (?returnTo=/room/business&from=business-zero-state; the form has no action) redirects after the save to /room/business?from=business-zero-state, and with a way back but no from= to /statements?returnTo=... (same-site only). new_account() also needs no OPEN action filed under Business (command_center street_actions, room "business", status new or in_progress), read in the one try. The upload card says "Add a CSV export from a distributor, PRO, publisher, or rights society." - a recorded DEPARTURE from spec 6's "CSV, spreadsheet, or PDF" because only CSV is read (the promise was fixed, not the upload: no PDF or spreadsheet statement path exists); a PDF or spreadsheet upload is refused by statements_engine.parse_statement with a plain sentence instead of its raw bytes as "Headers seen". "Ask Street Banker" (#bz-ask) opens the corner Ask box, /contact without scripts. On a phone (under 860px) the page from zero's five steps stack vertically, ring left and words right (business-room.css ?v=6, scoped to .bz-z-flow so the kit's rail stands elsewhere). The sidebar room links carry aria-current="page" on the room's own screen and "true" on a page inside it (every room, templates/base.html), and a ?returnTo=/room/<key> back link names the room as the sidebar does ("Back to Business", jinja global room_name), not "Back to the business room". STILL TO COME (spec 6, 9, 17, 22, 23): the review and confirmation step, duplicate detection, currency handling and undo-import are not built, so the desk says what is true instead (below); spec 23's eighteen analytics events are not sent - the app has no event pipeline, and building one is the owner's call.
...
- PRICED FROM THE TRACK'S OWN ROWS (make-it-real, 2026-09-23): a collection gap's dollar figure is a typical lane share (artist_os._LANE_SHARE) of what THAT track earned, from artist_os.earnings_by_track, which gives each statement row to one passport at most (by ISRC when the row carries one, by title when it does not, and to nobody when two passports share the title or the row's ISRC names another recording). A track with no rows of its own gets no figure. It used to be the whole account's statement total times the share, on every track, summed into an "Est. On The Table" headline that could exceed everything the catalogue earned; the headline is now a count of priced gaps and no total is printed. Each figure says its basis beside it. tests/test_real_money.py.
- Routes: GET /money-queue
- Files: app.py:7114; artist_os.py action_queue, lane_grid, earnings_by_track; tour_store.settled_income; touring.settlement_totals; templates/money_queue.html
...
- PUBLISHING AUDIT FIXES (2026-09-23). The add-song door's save now comes back to the room with its done line and the new song selected (see Track Passport CSV import for the form). "Import catalog", the spec's second link under "Your publishing catalog will appear here", is drawn - DOOR#pp-import, the shared form's CSV import - because one exists (it was left out as "no catalog import in the app"); it is a write door, so a read seat or a locked demo does not get it. The account chip's title on the page from zero is "The account this room is for" (no figures there); the drawer's heading is the summary's own text, read once; "Ask Street Banker" (id pb-ask) opens the corner Ask box, /contact without scripts. MLC evidence: a later failed ask never replaces a stored answer (db._attach_mlc_checks keeps the newest match or none over a newer error), and a failed ask with no answer reads "The MLC could not be reached" and "No registry has answered yet", never "Nobody has asked". Rights Conflicts is the Publishing room's alone for team seats. DEPARTURE, recorded (owner's to decide, audit publishing-4): the spec's section 4 keeps a song (work) apart from its recordings and releases; here one os_tracks row, a Track Passport, stands for a song and its single recording until a works table exists, so "Works on file" counts passports and the Catalog & passports lens no longer promises "its recordings".
...
- The sidebar line said "Income structure across your whole operation"; the page reads statement income and logged spending only (no tour settlements, sync fees or fan club payments), so since 2026-09-23 it says "Statement income against the spending you log."
...
- Access: Artist tier and above ("/conflicts" in _PRO_PATHS). Team seats: /conflicts is the Publishing room's own "conflicts" card, so a seat needs Publishing (until 2026-09-23 it was also in team_areas.EXTRA["business"], which won the tie by ROOMS order and shut a Publishing seat out of its own card - audit publishing-6).
...
- Because: /royalty-lanes is a 302 to /royalties#lanes (confirmed by test client); the matrix itself is royalties_desk.lanes() over artist_os.lane_grid for the account's own Track Passports (since 2026-09-23 a lane's estimate is a typical share of that track's own statement rows, none for a track with no rows, and the footer adds those per-track figures; a row belongs to one track at most, so the footer cannot pass the share of what the catalogue earned), and the walk fix separates "claimed" (a statement row pays it) from "connected" (set up, no income yet) so an unsold sync pack no longer reads as paying.
...
- The upload card names only what is proven (2026-09-23): it used to promise CSV exports from Symphonic, DistroKid, TuneCore, ASCAP, BMI and The MLC, and only a Symphonic export had ever been read (the others are header shapes in tests/test_statement_formats.py, not real files). It now says a real Symphonic export is tested, other services' files are read by their column names but not yet proven on a real one, and a file that does not read says what was missing.
- Before the file chooser (Business spec 6 and 22, audit business-5, 2026-09-23) the intake says: CSV exports up to 210 MB (MAX_CONTENT_LENGTH); the file itself is not kept, only its name and the rows it reads; no currency is converted; a row whose amount can't be read is skipped; Remove takes an upload and every figure from it back out; duplicates are not detected yet, so a file uploaded twice counts twice. A PDF, XLSX/ODS/Numbers or XLS file is named and refused (statements_engine._binary_kind). A successful upload keeps the door's way back: with ?returnTo= and ?from= it returns to the room with from= (the room's done line), with ?returnTo= alone to /statements?returnTo=.
...
- A stored master is a record (audit studio-5, 2026-09-23): studio_room.new_account() counts release_ready_store.stored_masters beside readings, tracks and covers. An account whose song was deleted after Release-Ready stored its master, or that stored one from the drawer without a song, met the page from zero saying "Nothing to review yet ... Masters waiting for your approval will appear here" with the master on file.
- A failed read is the error page (audit studio-3, studio-4, 2026-09-23): the analysis, tracks, covers, masters and the Clean Release loop are read in ONE try, as the other rooms do; any failure is templates/room_studio_error.html at HTTP 503 ("We could not load Studio", Try again, Open The Rack), never a fresh account and never "No master yet". Covers and masters fell back to [] and 0 on their own, which is what hid the covers NameError, and the other reads were a bare Flask 500. artwork_config.list_uploads still reads a folder it cannot list as empty, as it does for the Cover Studio.
- A read-only seat is told who adds songs (audit studio-6, 2026-09-23): can_add is True for the account holder or an edit seat and "seat" otherwise, the rooms' pattern, and zero_page() offers the song door only when can_add is True. A read seat with the Publishing room was handed the hero pill and the card button and refused at the save.
- The song door's way back works through the real form (audit studio-2, 2026-09-23): the catalog's add-track and CSV-import forms carry the door's returnTo and from as hidden fields, and a SAVED song goes back to returnTo with ?from= on (see Track Passport CSV import), so "Your first song was added. Its Studio workspace is ready." is reached without typing an address.
- Smaller fixes (audit, 2026-09-23): the five "What Studio keeps together" icons are gold, as ruled - `.sz-keep span` matched the icon's own holder and painted it the muted ink, and the text rule is now `.sz-keep > span:not(.sz-keep-ico)` (studio-9); the zero page's text-link doors (.sz-links a, .sz-more) are 44px targets (studio-12); the populated unit's empty bay says "Make a cover below." where it said "Add a track to begin." with a track on file (studio-11); "How the Rack measures" goes to /rack?returnTo=/room/studio#sb15 and the Rack opens the named unit's explanation panel on arrival (static/js/rackdsp.js openNamedUnit), where it landed on the top of the signal chain (studio-17). The header pill (.rk-cta, room-kit.css) is still 42px tall: it is the shared kit's, not this room's.
- Routes: GET /room/studio (dispatched from /room/<room_key>)
- Files: app.py room_screen() -> app.py _studio_room(); studio_room.py STEPS, clock(), decibels(), source_line(), title_of(), analyser(), needle(), covers(), path(), zero_page(), done_line(), showcase(), new_account(), build(); templates/room_studio.html; templates/room_studio_error.html (a failed read, 503); templates/partials/cc_rack.html (the page from zero); static/css/studio-room.css; static/css/command-zero.css (the plate's rules); static/img/studio-bus-plate.webp; static/img/room-plate.webp; rooms.py ROOMS["studio"]; stores db.py (track_analysis, os_tracks), release_ready_store.py, artwork_config.py
...
- Because: All three write or remove real files; ownership is the filename prefix (artup_<user>_ / aiart_<user>_), so another account's name cannot be addressed and 404s rather than being refused. /artwork/save accepts only https://image.pollinations.ai/ URLs and caps the download at 8MB; it serves the no-key Pollinations path only. With OPENAI_API_KEY set, /artwork/generate writes the aiart_<account>_<ms>.jpg file itself when the cover is made, so an OpenAI cover never passes through /artwork/save.
...
- Audit fixes (2026-09-23, releases-1 to releases-20): a release is a campaign row of type "release" (releases_room.releases_of) - a Fan Hub or Pre-save alone neither ends the page from zero nor says the done line, and the chooser opens on a release while still listing every campaign. The builder's save honours ?returnTo= (same-site, app._safe_next) and carries ?from= back, so the room's door returns to /room/releases?from=releases-zero-state and the saved release says the line. The write door is offered only to the account holder or an edit seat that can open /links/new (the Marketing room's page) on a plan that includes it: a Releases-only edit seat is told a seat also needs the Marketing room, a Fan plan is told a release needs an Artist membership (with See memberships) rather than handed a 402, and neither gets the working room's header door. The first-save list renders for every viewer (a read seat's "Release requirements" link had no target), and its target is the list inside the fold, so following the link opens it, with a small script opening it where a browser does not. Its arrow is 16px (it drew at ~199px). Track Passports, the Publishing room's card, is lent to the drawer under Release record (spec section 4), seat-gated like every tile. "Ask Street Banker" opens the corner Ask box. The Release check card reads "Twelve checks on your own release record before it goes out." (it said "The store checks"). A pasted-twice block of releases-room.css is gone.
...
- Audit fixes (2026-09-23, stage-2 to stage-19): the working room's cue figure says "No light show saved yet" (it said "No show saved yet" right under the line announcing the show). The passport version read is inside the room's one try, so a failure there is the error page at 503, not a bare 500; the error page offers "Back to Command Center" only to a reader who can open it (a seat without every room is sent from there straight back into the failing room), leaving Try again and Contact. From zero, the drawer's Stage Plot tile points at the first-show card ("The plot is drawn once a show exists. Shows start in the card above.") instead of opening the editor before a show exists, and it and the Tour tile draw their own icons. "What Stage keeps together" no longer claims every Stage tool reads the show record (the plot and the light show are one per account): "The date, venue, and city each show starts from." "Ask Street Banker" opens the corner Ask box (Contact only with scripts off); the five suggested questions stay plain text. The first-show form's rules are visible hints under the fields rather than placeholders a phone cut off. A one-off show's date must be a real calendar day (tour_os._real_day; 2026-02-30 used to be saved), and a refused date from the Stage form comes back to /room/stage?show_error=date#sg-z-show with the name, venue and city refilled from the session (never the URL) and the reason; without a returnTo the Tour desk answers /tours?one_off=date as before. The help panel's button stays inside its panel at every width (the text column's flex rule had caught the actions wrapper). Touch targets are 44px: the kit's header pill (room-kit.css, every room; Stage and Releases link it as ?v=10, the other rooms pick it up when their link is next bumped) and the two inline links under "Your shows will appear here". A read team seat is drawn the plot and input list without the Save controls (its save was refused by the team gate).
...
- The way back (audit studio-2, 2026-09-23): the add-track and CSV-import forms in templates/_catalog_passports.html carry a door's ?returnTo= (same-site, via safe_return) and ?from= as hidden fields. app.py _songs_saved_home() sends a SAVED song (a title, or at least one imported row) back to returnTo with ?from= on, so the room that sent it (Studio, Publishing, the Command Center) can say its done line, which it decides from the saved record; nothing saved stays on the passports page with both params still carried. An absolute or //host returnTo is dropped by _safe_next. Until then both forms dropped the two fields and always landed on the passports page, which offered "Back to Publishing".
- Routes: /tracks/import (POST), /tracks/add (POST)
- Files: app.py:6922 os_tracks_import(), app.py:6958 os_tracks_add(), app.py _CSV_PASSPORT/_passports_home()/_songs_saved_home() above; templates/_catalog_passports.html; db.py:3899 add_os_track()
- The shared add-song form (templates/_catalog_passports.html, since 2026-09-23, audit publishing-1/3/8): both forms post back the door's returnTo and from as hidden fields, and _songs_saved_home() answers the door there (returnTo same-site only, _safe_next) with its from - and, for the Publishing room, ?song=<the new id> - so the room's done line is reached through the real form; with no door it lands on the passports page as before. The add form takes a Writers field (passport songwriters, written only where the passport has none) or a "Writers not known yet" tick, one of them required by the form (the tick lifts the field's required). POST /tracks/add itself still accepts a title alone - the scripted and older callers - and reads it as not known yet; the tick records nothing either way: the passport then says Songwriters Not on file, which is true. The CSV import form carries id="pp-import", the target of Publishing's "Import catalog" link.- Access: Artist/Pro/Label; Fan 402; anonymous to /login. Publishing room; read-only team seat refused.
...
Turns a text prompt into an album cover plus a suggested colourway and layout. With OPENAI_API_KEY set the cover is made by OpenAI's gpt-image-1 and each account has a number of covers a month by plan; unset, it is the free Pollinations.ai URL exactly as before.

- Because: The image is real either way. With OPENAI_API_KEY (cover_ai.py, added 2026-09-23 on the owner's 'open ai and we allow x amount of renders a month'): POST https://api.openai.com/v1/images/generations with model gpt-image-1, 1024x1024, quality medium, JPEG, 120 s timeout, read from data[0].b64_json, and the bytes written to UPLOADS_DIR as aiart_<account>_<ms>.jpg, so the cover is already in the Cover Studio's files and the Studio cover bay. The allowance is cover_ai.DEFAULT_ALLOWANCE (fan 0, artist 10, pro 30, label 100 a calendar month, UTC), each number changeable by the owner at Settings > Cover art renders (app_kv cover_renders:<plan>, POST /admin/cover-renders, owner only, 404 otherwise). Every render is a cover_renders row reserved as pending inside BEGIN IMMEDIATE before the call, then done / failed / refused / timeout; only done and pending-under-10-minutes rows count, so a failed, refused (OpenAI code moderation_blocked) or timed-out render never uses a cover and two tabs cannot overspend. Owner logins are unlimited (rows written with counts=0, still in the owner's monthly total). A team seat draws on the account holder's covers because current_user() is the holder; a read-only seat is refused by team_seat_gate. Demo logins get 403 'Cover generation isn't part of the demo' and no render. At zero the page replaces Generate with a line naming the reset date. A remix has no seed on OpenAI: it is a new render and the page says it uses a cover. The owner's card shows this month's covers made across all accounts and an estimate at $0.042 a cover (OpenAI's gpt-image-1 1024x1024 medium price, checked 2026-09-23; prompt text tokens not counted). Without the key: the route builds a Pollinations.ai URL (no key, browser loads it directly) with a reusable seed for remixes, and nothing is counted. The 'suggestion' half is not a model - artwork_config.suggest_from_prompt is a hard-coded keyword-to-colourway table with a word-count rule for the layout, and its own note admits the seam.
- Routes: /artwork/generate (POST, JSON); /admin/cover-renders (POST, owner only)
- Files: app.py:4172 artwork_generate(), app.py:4150 _cover_who(), app.py:14709 admin_cover_renders(); cover_ai.py (render(), reserve(), finish(), status_for(), month_summary(), save_allowances()); artwork_config.py:86 suggest_from_prompt(), artwork_config.py MOOD_KEYWORDS; templates/artwork.html (allowance line, used-up line); templates/settings.html #cover-renders; tests/test_cover_ai.py
- Access: Artist/Pro/Label; Fan 402; anonymous 401 JSON (the route now checks sign-in itself). Studio room; read-only team seat refused (POST); demo logins never spend an OpenAI render.
...
- Because: The designer is real client-side SVG and the uploads listing is the account's own files read off disk by prefix. Finish of 2026-09-24 (the owner's mockup and audit, ticket 1 'words and states'): a four-step rail Art / Type / Canvas / Review & Export over cards that fold (each an h2 whose button carries aria-expanded; the rail names the active step with aria-current); one storage vocabulary ('Save to Studio Files', 'Studio Files are stored in your Vault.', 'Cover saved. View in Vault'); one sentence on formats (AI art is a square cover, Story and Banner are real reframes of the same composition, Motion for animation or video); three actions under the cover (Save to Studio Files, Download with PNG and SVG behind one menu button, Preview formats); Save / Apply brand kit moved into Type > Brand settings, localStorage only, 'Saved on this device' as visible words and 'No brand kit saved' in place of an Apply button when none exists; a 'Starter preview' pill while the cover is the untouched gradient, and a confirm() before saving that starter; Generate disabled until a description is typed (the prompt cap is 500 to match the 0/500 counter; it was 300); Variations 1 | 4 with aria-pressed, the 4 disabled with a reason when fewer than 4 covers are left this month; generating / done / failed written into #generate-note inside the Art card; four more look chips from the mockup (Abstract, Neon, Minimal, Urban) as real prompt words in artwork_config.LOOK_OPTIONS with a fifth row, Scene; the Fit-the-image sliders live under 03 Canvas. The slug under the cover is ink-2 (measured at or above 4.5:1 on the bench; the old rgba was about 3.9:1). The quick-fill title list is not: artwork_config.get_artwork_data pulls royalty_data.get_songs(), the invented Synthwave Surfer catalogue, for every account - the module docstring's claim that titles come from 'the live catalog' is contradicted by the code.
- Ticket 2 (2026-09-26, the owner's 'build ticket 2'): the 04 Review & Export card holds three boxes the page ticks from state and never by a click (disabled inputs, aria-checked set by updateReview() on every render): Artwork added (the canvas is not the starter gradient), Release information (a title and an artist), Store-ready check (the last 'Check this cover' passed on THIS canvas: state.checkedSvg is compared with buildSvg(false), so any change clears it). Cover status reads Ready only when all three hold, and the rail's step 4 carries data-ready. 'Check this cover' renders the canvas at export size (coverPngBlob) and sends it to the same POST /artwork/check as a chosen file, with no file dialog; the file path stays as 'Check another file'. 'View saved file in Vault' appears once a save has happened. Test: tests/test_cover_studio.py::test_the_review_step_checks_the_canvas_not_a_file.
- Ticket 3 (2026-09-26): every look row's 'clear' button carries aria-label 'Clear <row>' (Clear Light, Clear Framing...); letter spacing, the four position sliders, the two colour pickers and the logo controls fold under a details 'Fine-tune placement' in the Type card (closed by default; title, artist, typeface, text style, alignment and the two sizes stay visible; every id kept); Preview formats is a horizontal tray (.cs-tray, 150px cells, overflow-x) under the cover instead of a three-up grid, so the square cover stays in the sticky bench; .studio carries 96px of bottom padding so the floating Ask button never covers the last control. Already in place from ticket 1: the Art card's generating / done / failed line, the h2 headings on the four cards, aria-pressed on Variations, aria-expanded on Preview formats, and the slug's contrast (var(--sb-ink-2)). Test: tests/test_cover_studio.py::test_ticket_3_named_clears_fine_tune_fold_and_the_tray.
- Routes: /artwork
- Files: app.py:4039 artwork(); templates/artwork.html; artwork_config.py:73 get_artwork_data(), artwork_config.py list_uploads(); royalty_data.py:375 get_songs(); artwork_config.py LOOK_OPTIONS / LOOK_GROUPS; tests/test_cover_studio.py
...
- Files: artwork_check.py (check(), unavailable(), SPEC, UNCHECKED); app.py:4106; templates/artwork.html:299-317 and :882; requirements.txt (Pillow, added for this; numpy==2.5.2 added 2026-09-26, without which the sharpness and not-blank rows said Not measured on Render); tests/test_artwork_check.py (measurements) and tests/test_artwork_check_wired.py (the route)
...
- Because: app.py:11295 def ml_fans_export() writes a real CSV from links_store.list_fans() and skips rows whose `suppressed` reason is set unless ?include=suppressed; probed, returned the header plus the account's real rows. The Visits and Clicks columns count what a known fan did (see "Fan engagement counters").
...
- Because: app.py:2043 smart_link_redirect() resolves the campaign, 410s an archived one, 404s a draft for anyone but its owner (owner_preview writes no events), writes a page_view ml_events row (carrying the fan's id when the request carries their signed ?f=; see Fan engagement counters) and renders link_campaign.html with the real destinations. Before release the email box says "Remind me" where the release-day email can go and "Join the list" where it cannot (links_engine.notify_button_text; an artist's own button text is kept unless it claims a save), its reply promises the email only where it can go (notify_done_text), and the service rows say "Open"; until 2026-09-23 the box said "Pre-Save" and the Spotify, Apple Music and Deezer rows said "Pre-save" (audit overclaim 17). The real Spotify pre-save button stays separate and appears only with the owner's Spotify app configured; a slug that is not a campaign falls back to db.get_db_link + store.log_click. Probed indirectly: subscribing through a published slug worked and the events landed.
- Routes: GET /l/<slug>
...
Per-fan Visits and Clicks in the Fan CRM table, its CSV export and the intent score, counted from what a known fan does on the account's smart links.

- Because: a fan is known on a smart link when the request carries their signed ?f= (fan_mail.fan_token, itsdangerous salt "fan-link", no expiry): the listen link in each release-day email app._send_release_emails sends them, which the page passes on to its own service buttons (templates/link_campaign.html, a[data-go]). POST /l/<slug>/subscribe hands no token back (until 2026-09-23 it did, so anyone who typed a fan's address could press the buttons on that fan's record). app._link_fan() accepts it only for a fan whose user_id is the campaign's owner, and never on a HEAD request (mail scanners HEAD every link; Flask answers HEAD with the GET view). A known fan's page view is recorded with their fan_id and adds one to total_visits unless they already viewed that campaign within fan_mail.VISIT_WINDOW_MINUTES (30), so a reload is a view but not a second visit; a /go/ press adds one to total_clicks. Both go through app._credit_fan(), which re-scores intent (visits 2 points each up to 10, clicks 5 each up to 20). Anonymous views and clicks, the owner's draft preview, a token for another account's fan and a forged token credit nobody. The CRM table shows both columns under a line saying what they count; the CSV reads the same two fields. Nothing is stored on the fan's device: the token rides in the link, and the page script strips it from the address bar so a copied URL is the plain page. Until 2026-09-23 nothing wrote either counter and the CRM table had dropped both columns. fan_audience._clicked() (clicks or pre-saves) still has no caller.
- Routes: GET /l/<slug>?f=, GET /l/<slug>/go/<dest_id>?f=, POST /l/<slug>/subscribe, GET /links/fans, GET /links/fans/export.csv
...
- Because: the release-day email and the Fan Club drop notice each carry the recipient's own unsubscribe link, /unsubscribe/<token>, where the token is fan_mail.unsubscribe_token() (itsdangerous, salt "fan-unsubscribe", no expiry) naming the artist's account and the one row the email went to, the fan's ml_fans record (fan_mail.FAN_REF) or their club_members row (MEMBER_REF), never the address (signed is not encrypted; the address is looked up from the row when the link is used, and a link whose row the artist has since removed says nothing is sent), plus List-Unsubscribe and List-Unsubscribe-Post: List-Unsubscribe=One-Click headers (RFC 8058) passed through email_provider.send(headers=). GET shows one button and changes nothing (mail scanners follow links); POST, from that button or the mail client's one-click POST, calls links_store.suppress_fan(owner, email, "unsubscribed") and notifies the artist. A Fan Club member with no CRM row gets one through links_store.add_suppressed_fan(), dated from their membership so they are not a new fan today. The CRM's per-row Do not contact (POST /links/fans/<id>/do-not-contact) writes "do not contact"; Allow contact (POST /links/fans/<id>/contact-again) lifts only that reason, and the unsubscribe page's Subscribe again lifts only "unsubscribed" (links_store.unsuppress_fan(only_reason=)), so neither side can undo the other's. A fresh sign-up on a smart link (POST /l/<slug>/subscribe) is consent given again and lifts only the fan's own "unsubscribed"; under the artist's mark the sign-up is recorded, the reply promises no email (links_engine.notify_done_text(False)) and none goes. The release-day list goes through fan_segments.contactable() and the drop loop skips fan_mail.suppressed_emails(). Until 2026-09-23 nothing in the app wrote the column.
- Routes: GET/POST /unsubscribe/<token> (public), POST /links/fans/<fan_id>/do-not-contact, POST /links/fans/<fan_id>/contact-again
- Files: fan_mail.py (unsubscribe_token, read_unsubscribe_token, unsubscribe_headers, suppressed_emails, footer_html, UNSUBSCRIBED, DO_NOT_CONTACT); app.py fan_unsubscribe(), _unsubscribe_address(), ml_fan_do_not_contact(), ml_fan_contact_again(), _fan_unsubscribe_url(), _send_release_emails(), fan_club_drop_post(), "/unsubscribe/" in _PUBLIC_PREFIXES; links_store.py suppress_fan(), add_suppressed_fan(), unsuppress_fan(); db.py get_club_member(); email_provider.py send(headers=), release_email_html(unsubscribe_url=); templates/fan_unsubscribe.html, templates/links_fans.html, templates/fan_club.html; tests/test_fan_unsubscribe.py
- Access: /unsubscribe/ is public; the signed token is the authorisation and names one fan record or membership on one account, from which the address is looked up. The CRM marks are Artist tier or higher, own fans only (another account gets 404); a read team seat is refused by team_seat_gate.
...
- A door that says where it came from gets the person back there (audit releases-2, 2026-09-23): POST /links/new with a same-site ?returnTo= redirects to it, adding ?from= when the door carried one, instead of to /links/<id>/edit. This covers the Releases, Marketing, Fans and Command Center doors; without returnTo the builder still opens the new campaign's edit page.
...
- Because: links_engine.py:160 calculate_fan_intent() weights four counters, and the captures and pre-saves halves are really fed (bump_fan is called with total_presaves at app.py:2005 and app.py:2115 and with total_captures at app.py:2115 and hypeddit_ingest.py:301). The visits and clicks halves (30 of the 100 points) are fed since 2026-09-23 by app._credit_fan() when a known fan views or clicks through their own signed link (see Fan engagement counters). The band "Cool" that fan_audience.INTENT_ORDER and INTENT_COPY describe is never produced by this function either; only the showcase generator writes it.
- Routes: none of its own; called from /l/<slug>/subscribe, /presave/callback, the Hypeddit receiver, and GET /l/<slug> and /l/<slug>/go/<dest_id> for a known fan
...
- Because: Every figure is real — fan_room.build() reads the same rows the Audience screen reads (links_store.list_fans + fan_audience.for_account), the tile statuses are true counts (fan-crm reads audience total, fan-club reads active club_members, marketplace counts open collab_requests), and the "Get their emails" move hands over a real CSV. The "fan lifecycle" rail counts each stage from the account's own records (fan_room.lifecycle(), since 2026-09-23; until then it was five fixed lines with nothing counted): Discover is the smart link visits the Audience funnel reads (ml_events page_view, views not people), Capture the fans on file, Know those with a name, city or country, Activate those who pressed a service button on a smart link (total_clicks; a sign-up before release is a capture, and most pre-saves are email reminders), Belong the active Fan Club members, or "No Fan Club yet". A stage with nothing counted says so in words, never 0 (tests/test_fan_room_lifecycle.py). The "Find N missing emails" move can never appear (no write path produces an empty-email fan).
...
- AUDIT FIXES (2026-09-23, audit-group-mkact): THE DONE LINE IS REACHED BY THE REAL FORM - the door carries `from` inside returnTo (/room/marketing?from=marketing-zero-state, the Analytics room's shape) and ml_new's save carries returnTo and from through to /links/<id>/edit (same-site only, through _safe_next), so that page's shell back link is the way back to the line; the line is still said only by a saved campaign. A FAILED PRESS KIT READ is the error page at 503 like every other read (store.get_epk moved inside the one try; it was a bare 500). ONE DOOR CHECK: app.py _door_check(user) gives the room a can_open that refuses what the request would refuse - a team seat's _TEAM_BLOCKED pages (/referrals, the account holder's alone, was offered to every seat), its rooms, and a page the owner switched off (page_switch_gate; an owner passes). marketing_room.build() now drops a closing tile the reader cannot open, as the zero drawer did, and zero_page() runs the gold door through the same check: with Smart Links switched off the door is the words "The campaign builder is switched off for now..." instead of a bounce to the Command Center. The card's claim that "Street Banker shows the smallest useful toolset" is gone (the builder narrows nothing); the goal step itself waits for the owner. The hero's two closing divs are emitted in both states; the Smart Links drawer tile draws the link icon (the macro knew only "link"); marketing-room.css no longer redeclares the kit's .mk-hero-aside (it floated the chip mid-header at 768; tests/test_room_kit.py SHARED now names hero-aside), the zero page's duplicated block is gone, and the goal and checklist lists carry markers. marketing-room.css is ?v=8. Locked by tests/test_marketing_room.py test_marketing_* (the real-builder path, the three failed reads, route-level seats of four kinds GETting every door, Smart Links and Rollout switched off) and the demo test, which now signs a real demo session in and asserts unconditionally.
...
The first page view after release day, or the daily run, within a week of release, emails every consented fan of that campaign the listen link.

- Because: app.py:1679 _send_release_emails() is real. It sends nothing for a campaign with no release date or one released more than links_engine.RELEASE_EMAIL_DAYS (7) days ago (release_email_window_open(), checked here for both callers since 2026-09-23; before that only the daily run kept the window, so a page view mailed "out now" months late). It claims the once-only release_email_sent flag with one conditional UPDATE (links_store.claim_release_email(), json_set where the flag is unset) and sends only when its own claim landed, so a first page view during the daily run, or two views at once, cannot mail a fan twice (until 2026-09-23 the flag was checked on the caller's copy of the campaign, and a view mid-run double-sent). It then sends one message per fan in fan_segments.contactable(links_store.campaign_fans()), so a fan who unsubscribed or was marked do not contact is left out (since 2026-09-23; before that the list had no suppression filter). Each message is the fan's own: its listen link carries their signed ?f= (see Fan engagement counters) and its foot carries their signed unsubscribe link, with List-Unsubscribe and List-Unsubscribe-Post headers (see Suppression). POST /reminders/run also sends it for any live campaign released in the last RELEASE_EMAIL_DAYS (7) whose flag is unclaimed (links_store.released_unsent_campaigns, re-checked by the claim), so a fan who asked to be reminded is not waiting on somebody opening the page; an older release is left alone rather than mailed "out now" months late. Every link in the message, and the List-Unsubscribe header, is built on the artist's public address (app._fan_mail_base(): the artist's reseller's own domain when it has one, otherwise PUBLIC_BASE_URL, https), never request.url_root, and the sender name is the artist's reseller or the platform (app._fan_mail_from(), email_provider.send(from_name=)); until 2026-09-23 the daily run's mail carried the scheduler's host and the platform's name. The Fan Club drop notice builds its links and sender name the same way. It only runs when RESEND_API_KEY is set (emailer.configured()); tests/test_link_notify_wording.py, tests/test_fan_unsubscribe.py and tests/test_fan_counters.py exercise it against a patched transport.
- Routes: triggered inside GET /l/<slug> (no route of its own)
- Files: app.py:1679-1703, called from app.py:1708 _process_due_presaves() and reminders_run(); links_engine.py RELEASE_EMAIL_DAYS, release_email_window_open(); links_store.py claim_release_email(), released_unsent_campaigns(), :452 campaign_fans(); email_provider.py:28 configured(), release_email_html()
...
- Access: Showcase session only (_session_is_demo, i.e. the four seeded addresses in demo_accounts.ACCOUNTS). Until 2026-09-21 it was served unlabelled to every visitor including real fan accounts, which is the honesty defect this entry records as fixed. The sidebar stamped Discover "Sample" for every account until 2026-09-23; "discover" is in hubs._BASE_LIVE now, because a real account's page is real search and an honest empty state, and the showcase session's feed carries its own "Sample feed" banner on the page.
- Access: Showcase session only (_session_is_demo, i.e. the four seeded addresses in demo_accounts.ACCOUNTS). Until 2026-09-21 it was served unlabelled to every visitor including real fan accounts, which is the honesty defect this entry records as fixed. Since 2026-09-23 "discover" is in hubs._BASE_LIVE, so neither the sidebar nor the Fans room's tile stamps a real account's search "Sample"; the example feed is labelled on the page itself for a showcase session.
...
- Since 2026-09-23 (audit): status reads Not started / In progress / Complete / Dismissed (command_center.STATUS_LABELS, through the chip macro's label) and the button says Complete; the Actions link shows only when the viewer can open this account's /actions (tour_os._actions_open); the Assignee field no longer lands in the title - a confirmed team member is the assignee, anyone else is "Tour crew: <name>" (street_actions.assignee_name).
...
- AUDIT FIXES (2026-09-23, audit-group-mkact): a returnTo carrying any control character is refused (app.py _safe_next and command_center._safe_href; a browser strips a tab, so /<TAB>/host is //host and was stored as the way back and drawn in the shell's back link). A failed read of the actions is templates/actions_error.html at 503 ("We could not load your actions", Try again / Command Center) on /actions and /actions/<id>, and the Command Center's error page at 503 on both its pages (their open_actions reads were outside the guard). A records store that fails is named in actions_center.Records.failed, and a record of that kind reads "This <kind> could not be read right now", never "no longer on file"; the edit form keeps its link. The operational Command Center counts every open action ("8 open actions on your board, the first 5 listed under Open Actions"). An empty board says so once: no List/Board toggle, no zero-count pills, no second sentence. The demo accounts open on a showcase board (demo_seed.seed_actions: eight rows, two not started, two in progress, three complete, one dismissed; written once at boot, keyed on created_by = "demo-seed"). "Action deleted" is said only after a delete (the title rides in the session; ?deleted=1 carries nothing). Creating from the Board stays on the Board. "<noun> raised this action" reads as a sentence (command_center.SOURCE_NOUNS), and the list's menu note names the real source. Step buttons and Open doors carry aria-labels naming their action. CSS (actions.css ?v=3): the overflow menu opens from the row's right edge and never wider than the screen; the summary figures wrap without a lone divider starting a line; a long unbroken title wraps in the detail plate and on the Command Center from zero (command-zero.css .cz-h, loaded there as ?v=5). Tour tasks read the same state names and "Complete" button, link to Actions only for someone who can open this account's list (else "Also on the account's Actions list"), and keep the assignee off the title: a confirmed team member by name or email is the assignee, anyone else is kept as street_actions.assignee_name (new column) and reads "Tour crew: <name>" on both pages. Still open, the owner's: "today" is the UTC date, because no account has a timezone on file. Locked by tests/test_actions_center.py test_actions_*.
...
- /all-tools (GET, artist tier): the complete 33-window directory, grouped like the sidebar, with an in-page search - moved out of the Command Center per the spec ("do not place the full directory back inside"). Both pages read command_center.directory(), not MODULES itself: the Fan Club line says "Set up your club now; paid joins open when online sales are switched on." until the owner's online-sales switch and Stripe are both on (fan_club_joins_open(), the two checks club_join makes), and only then "Paid monthly memberships through Stripe..." (since 2026-09-23; tests/test_fan_club_directory_line.py). The Command Center still draws its board until Pass 2 replaces it with "Explore at your own pace -> Browse all tools".
...
- THE REST OF THE 2026-09-23 AUDIT (group cc). THE WAY BACK IS REAL: every door is a step's `door` (account_state.build), so the shell's back link is /command-center?from=<key> and the done line is reached by following it; the saves carry it too - the catalog's add-track form posts the door's returnTo and from=, and POST /tracks/add redirects to it (same-site only, _safe_next); POST /links/new carries ?returnTo=/?from= on to /links/<id>/edit. Capture's door opens the first campaign not yet capturing at its capture section (/links/<id>/edit#capture, links_builder.html section#capture), not the /links list with no capture control on it; the Rack's door is /rack?track=<the newest song with no audio> (account_state.account_doors, read in _account_state's one try). IDENTITY is an artist or label DETAIL: the Press Kit's settings (account_state.EPK_SETTINGS: bg_color, show_sweep, sections_off, sections_on) do not count, so an untouched Save - which always sends the colour input's default - no longer completes the step or says "Your profile was saved". A READ SEAT gets no setup door: no Continue setup, each step says "The account owner or a seat with edit access does this step.", no In progress door and no Publish/capture priority door (an Open action door stays); an edit seat keeps every door. EVERY FAILED READ IS THE 503 PAGE: the signal profile, the open actions, the campaigns and songs, the alerts, the summary and the money band are read in one try with the state's reads, and any raise renders command_center_error.html at 503 instead of Flask's bare 500. THE ICONS carry .sb-i and command-zero.css gives .cz svg.cz-ico fill:none / stroke:currentColor, so the wave ring and the Done tick are drawn and the globe, folder, compass and speech are gold lines, not black shapes. A LOCKED step is a real disabled <button>; Done is a state in words (no aria-disabled on a span); the operational panel's locked control is a disabled button too. THE OPERATIONAL PAGE: its h1 comes first (the title block now sits above the rack); the Start-here panel never says "completely empty" (the page is operational mid-setup only because records exist); the money band draws no "vs last month" arrow, percentage or Last month/Growth footer with one period on file - it says "One period on file, so there is nothing to compare it with yet." /all-tools cards are h3 under the group h2s (sb.module_card level=). THE EXPLORE PANEL puts its button under its words, so its heading no longer breaks into four lines at 1280. DOORS A READER CAN FOLLOW (x-1, x-2): app.py _door_refusal(href, user, seat) answers "seat" for a page every seat is refused (_TEAM_BLOCKED) or a room the seat was not given, and "off" for a page the owner switched off, for anyone but an owner (/tracks judged where it lands too); _room_can_open() hands it to all eight rooms as can_open (a callable with .why), rooms.gate_zero() turns a room's fixed primary door into "This is switched off on this account for now." and drops help links and starting points that would bounce; the Command Center's steps, Continue setup and Browse all tools follow the same rule; the Tool suites strip and /all-tools list no door a seat is bounced from. command-zero.css ?v=5 on the Command Center. tests/test_command_center_audit.py.
...
Turns a Command Center alert, a release check, a rights conflict, a catalog or score finding, or a preview page's reminder into a high-priority action that remembers where it came from, and confirms it on the page you were on.

- Because: the handler writes through cc.create_action into street_actions with the source the form names (alert, release_check, rights_conflict, catalog_check, growth_score, trust_score, module) or, failing that, the one actions_center.source_for_path reads off the page it sits on; the room the form names, else the page's room, else the action type's room; the page as the way back; the release or campaign the release check names, kept only when it is this account's; and the person who pressed as the assignee. It redirects back to the referrer with ?action=<title>&action_id=<id>; partials/action_created.html adds "View action" only when that id is this account's saved action. An empty title is refused before any write (locked by tests/test_walk_command-tools_2026_09_20.py and tests/test_actions_center.py)
- Routes: /actions/from-alert (POST)
- AUDIT FIXES (2026-09-23): a Command Center alert's form carries its Fix now link (`fix`, same-site only); it decides the room before the page or the action type, becomes the way back, and a /links/<id>/edit link names the campaign, kept only when it is this account's (a consent-copy alert was filed in Publishing with no record). The Rights Conflict Center offers Create action on every row: "Review split conflict: <song>" (and the other kinds' words, rights_conflicts.ACTION_WORDS), source rights_conflict, room Publishing, the song as the record, kept only when the passport is this account's; a seat that cannot file an action is told who can. The "royalty_check" source is gone: its two forms sat in a _real_royalty_band.html mode no page set, and on the Recovery desk a case is the action.
- Files: app.py:7005 action_from_alert; command_center.py:116 create_action; actions_center.py source_for_path; templates/command_center.html, templates/release_autopilot.html, templates/catalog.html, templates/qualification.html, templates/trust_score.html, templates/conflicts.html, templates/module_preview.html (the forms); templates/partials/action_created.html
...
- AUDIT FIXES (2026-09-23, audit analytics-1..15): THE WAY BACK: Connections reads ?returnTo= (same-site, _safe_next) and puts it on every action link, before any #fragment, so the pin made on /pulse?returnTo=/room/analytics?from=connect comes back with the done line; before, every action was a bare path and the line showed only via Connections' own back link. The done line also counts an uploaded statement as the saved source. PROVIDER READINGS: the route reads the metrics provider's STORED readings with _provider_metrics(fetch=False) inside the one try (no provider call, no quota), and build(metrics=..., metrics_label=...) credits monthly listeners - and followers, when Spotify's snapshots carry none - to the provider's label with the day each was read; an absence says "Needs a metrics provider key" only when the server has none, otherwise "<provider> has no reading for this artist yet"; audience geography with a provider set up says it is not stored for this room and Artist Pulse asks the provider for it. The raw key ("soundcharts") is never printed. BRAND-NEW (spec 8): new_account() also needs no uploaded statement, no observation other than insights_engine's "start" tip, and no open action filed under Analytics; statements and those actions are read in the one try, and when the observations cannot be read and every other count is empty the room is the error page, because "no insights" cannot be confirmed. A failed observations read is said as one ("Observations could not be read just now.", the path's Read step "Could not be read"), never "Nothing to read yet". The error page's "Review connections" carries returnTo=/room/analytics. CONNECTIONS (/connections, the card's door): a failed profile, statement or tour-date read is that row saying "Could not be read just now" with a "Could not be read" lamp, not a 500 and not "Not connected"; new rows for the metrics provider (its label when set up) and The MLC (Connected when sp.mlc_adapter().configured(), action /recovery#mlc), and the MLC leaves "Not connectable yet" when it is set up (ASCAP and BMI stay); MusicBrainz is gone from the public-APIs row (credits come from The MLC since 2026-09-18); the Spotify-profile line no longer says followers and popularity "track daily" (Spotify stopped sending them). The pin, the metrics provider, The MLC and statements each carry "What this source gives": gives, does not give, access, how it arrives, to disconnect - as they are today (spec 3). THE PAGE FROM ZERO keeps the account chip (artist_identity.display_name) as Business does; "Ask Street Banker" (#an-ask) opens the corner Ask box, /contact without scripts. A read seat is offered no Change artist / Pin your artist on the working page either. DRAWER: ZERO_TILES stays the room's four cards (Artist Pulse, Growth Score, Artist Twin, Reports), a recorded DEPARTURE from spec 12's list (Detailed reports and Exports are the one Reports page, Comparisons are Artist Pulse's peers, Source health is Connections); whether to follow the spec's list is the owner's call.
- Open actions (audit, 2026-09-23, x-5): the spec's "brand-new only after confirming ... no Analytics actions" is kept - an open action filed under the Analytics room (street_actions.room = analytics, status new or in progress) keeps the room off the page from zero, read in the same one try as the other counts, so the action's "Open Analytics" door no longer lands on a page that calls the account brand-new. The other rooms' specs do not name actions; whether their pages from zero should show a room's open actions is the owner's call (Business's is business-2).
...
- The All Tools line (command_center.MODULES) said "Live Spotify followers, popularity, top tracks, and Deezer fans". Spotify retired followers, popularity and genres for apps like this one (2026-09-15), and Deezer fans and top tracks need SPOTIFY_CLIENT_ID/SECRET, so since 2026-09-23 it names what the page reads without them: followers and monthly listeners from Soundcharts, a YouTube channel the owner names, and clicks on the account's own smart links. The sidebar line said "Daily follower and popularity snapshots"; readings are taken when the page is opened, so it says "Follower and listener readings kept over time". The Artist Twin's audience section now reads followers alone (it waited for a popularity reading that no longer comes) and no longer promises popularity or Deezer fans. With fewer than two Spotify follower readings it reads the metrics provider's stored series (Soundcharts, written when /pulse is opened), because the live site has no Spotify keys and the Spotify series is never written there (app.py artist_twin_page; tests/test_real_money.py).
...
- MONEY LEFT ON THE TABLE (make-it-real, 2026-09-23): for a real account the card prints two figures side by side, Actual (recovery_engine's actual_unattributed: paid, no track named) and Estimate (estimated_gaps: stores silent on a track), and never the sum. It used to lead with total_at_stake, which adds the two. The same sum is gone from the Valuation driver (its chip reads "$X actual", or "about $Y estimated" when there is no actual money, instead of "$X at stake"; the detail line names both) and from POST /scan/missing-royalties, whose total_estimated is now the estimate alone beside a new total_actual. tests/test_real_money.py. The showcase login's version of the card, which reads royalty_data's sample findings, carries a Sample tag on its face like the ledger line above it.
...
The three memberships on the rooms' slim three-screen plate, each screen a door to Billing, with the credits plate beneath them.

- Because: split_home.get_split_home_config() reads every price from plans.PLANS at render and coming_soon() from hubs.suites_pending(), so the glass cannot drift from Billing; the band is drawn by partials/memberships_band.html (rack, or the engraved passes on MEMBERSHIP_BAND=passes). 2026-09-26: the plate is room-plate.webp at cc_rack.html's measured boxes (owner: "use this one ... instead of the thicker, wider one"); each screen shows the tier's name at rest and the full reading cuts in on hover, focus or a first tap (owner: the empty glass "looks like an error"); the credits band is back under the rack (split_home.SHOW_CREDITS), drawn on the owner's photographed credits plate (static/img/credits-plate.webp, 1508x562; owner: "Use this plate for the home page for credits. You fill out the needed text and button placements on it") with live words on its measured black field: "Studio credits", the monthly figure read from plans.LABEL_MONTHLY_CREDITS (what Billing grants Label each month), one line saying what credits do ("Credits unlock The Room for audio and Motion for video.", true by plans.suite_open; the owner struck the earlier compute and Noise Lab wording on 2026-09-26), and two doors (Choose Label to /billing, Compare memberships to #memberships); below 1100px the words step under the plate; still no pack prices while plans.CREDIT_PACKS_ON_SALE is False.
...
- Routes: POST /epk/save; POST /epk/bio (the bio alone, merged into the saved kit); POST /epk/photo; POST /epk/photo/delete; POST /epk/asset/<kind>; POST /epk/asset/<kind>/visibility; POST /epk/asset/<kind>/delete; POST /epk/asset/<kind>/from-vault
...
- Because: readiness.py init registers GET /admin/readiness; it redirects an anonymous visitor to login and abort(404)s a non-owner (probed: 404 for a plain account, 200 for an owner). Rows are built by _try/_call wrappers so a raising provider is reported rather than 500ing the page, and the module docstring states the rule it holds: a key is a presence check, not a capability. The drift the 2026-09-23 providers inventory found is fixed: The MLC and Discogs rows ask the adapter's configured(), so each needs its MLC_ENABLED / DISCOGS_ENABLED switch (and Discogs stays off in a sandbox) instead of reading the credentials alone; ACRCloud is two rows, identify (host, key, secret) and console (ACRCLOUD_CONSOLE_TOKEN), each with its own lamp; the Signal rows name SOUNDCHARTS_ACCESS_TOKEN (token_key); Ticketmaster reads ticketmaster_provider.configured(), so it is off while the owner's ruling keeps it off, and Google Maps reads venue_geo.configured() (sandbox-aware); the /presave/diag wording says it runs the client-credentials exchange, and that its token can come from the 50-minute cache. The page links the Providers page, which asks each service a real question. 22 tests in tests/test_readiness.py.
- Routes: GET /admin/readiness
- Files: readiness.py; templates/admin_readiness.html; registered from create_app
- Access: Owner only (_is_owner_email), 404 to everyone else; blocked for team seats by the /admin prefix in _TEAM_BLOCKED; a reseller's staff acting on an account's behalf never reaches it as the owner, since current_user drops an act-on-behalf of an owner's account (providers review, 2026-09-23)

**Providers status page**

One owner page asking each outside service the app depends on whether it answers right now, with its keys shown by name only.

- Because: provider_status.py lists every outside service (PROVIDERS, in six FAMILIES: music data and rights; touring and places; money, shop and mail; storage, backup and monitoring; audio tools; public music lookups) with the variable names it needs, its switches, what it powers and the page it feeds, and one check that goes through the app's own adapter below any cache: The MLC's search by a known ISRC through mlc_adapter()._call_raw (sign-in alone is not proof; only a 204 or a 200 list is working, an unreadable 200 is failing), the Soundcharts adapter's _fetch_json through soundcharts_budget, SongstatsAdapter._get('/artists/search'), discogs_adapter()._send('/oauth/identity') after the adapter's own _pace() (and nothing asked while the 60-a-minute window is nearly spent), the Spotify client-credentials grant through spotify_provider._http, YouTubeAdapter._fetch_json('/channels', part='id') with the key redacted (never /search), acr_console._call GET /api/buckets, the registry's own MusicBrainzAdapter._get (so the check waits its turn on the one-a-second pacing, now held under a lock), Eventbrite's organizations through eventbrite_provider._http, a size-1 Discovery search through ticketmaster_provider._http (never while switched_off()), Places/Geocoding/Time Zone/Routes through venue_photos._http and venue_geo._http (never drive()), Stripe's webhook endpoints through stripe_provider._http_get (never setup_webhook_endpoint, which POSTs), shopify_buy.check(fresh=True) (never the Storefront mint), the Admin API's { shop { name } } through shopify_customers._post (never forget_grant), email_provider._api_get('/domains'), blob_store.round_trip, the ElevenLabs SDK's models.list with a 10-second limit and no retries, roex_client.health(), a raw GET of StemSplit's /balance with stemsplit_provider's own host and headers (so Cloudflare's plain-text "error code: 1010" reads as the firewall, not the key), and the keyless music_apis lookups (Apple, Deezer, Odesli, Google News). Each answer's shape is read, so a 200 whose JSON is an error is failing, not working. A result is working (True), failing (False, in the vendor's own words) or not asked (None: not configured, off by an owner ruling or in a sandbox, or no safe check), and the lamp never reads "not asked" as failing; a known state a service named when asked gets its own label (Google Routes: "Not enabled" for SERVICE_DISABLED, "Blocked by the key" for API_KEY_SERVICE_BLOCKED, and such an answer counts as asked for the row's once-a-minute interval). Stripe counts only the webhook endpoint on this deployment's own host (the press's request.url_root, else PUBLIC_BASE_URL), and no endpoint here, a disabled one, and one whose signing secret the app does not hold all read failing; a legacy secret the route still accepts (webhook_accepts) reads "held by the app (legacy)". Shopify Admin reads a grant failure by what it answered (a refusal, a 429, a server error, or no answer). Shopify Buy Buttons with the Admin credentials set and no Storefront token read "Not minted yet" (the app mints one on the first Apparel visit), with the last mint refusal if there was one. Nothing is called on a page view. Results are kept in app_kv under provider_status:<key>; "not configured" and "off by ruling" answers are never stored, so none outlives its setting. Every word is scrubbed before it is stored or shown, and the scrub is the only cut: every configured value (as typed, URL-encoded and JSON-escaped), the secrets the app holds in app_kv or memory (the Shopify Admin grant token, the minted Storefront token, each Stripe signing secret, Soundcharts' kept bearer, The MLC's bearers), the start of any credential however soon an adapter cut it (8 characters or more), credential headers echoed as a dict or a line, and bearer/basic/key= shapes and a vendor's masked sk_ echo. Each check runs in its own thread under a hard time limit (12 seconds; The MLC 20, Songstats 15, Soundcharts 35, R2 45 - each above its adapter's own worst case), and a costed check past its limit says what it already spent (Soundcharts: the month's count). Check all runs the free set together, bounded by the slowest limit rather than the sum. One check per service at a time, and one Check all at a time, across workers: a lease row provider_status:running:<key> claimed in one statement and let go after (it lapses by itself), so a double-click spends once (Check all claims its leases, reads the last results, reads the held secrets and saves in one database round trip each, and a page view reads every row's last result in one read); a press that finds one running, or lands inside a service's interval, shows the last answer and the page says which service was not asked again and why. Rows with no safe live check show what the app already keeps: the stored backup_last_run (failing when a run failed or is older than 26 hours), the newest beat_fingerprint_checks row for ACRCloud identify, the newest distribution application for Symphonic (every call files a real one), whether Sentry's SDK started in the worker, which inbound webhook secrets are held, and presence for Resend inbound and the suite sign-in secret. Bandsintown reads Dormant; the Chartmetric, SoundExchange and Spotify-metadata stubs read Not built and show no key. The R2 round trip deletes its test object even when the write's reply is lost or a 5xx comes back (blob_store.round_trip), and says when the delete is not confirmed. 306 tests in tests/test_provider_status.py (and the round trip's own in tests/test_blob_round_trip.py), against a fake wire in urllib and the SDK client with a socket guard behind it.
- Rule: "Check all free services" runs only free, read-only checks that write nothing: The MLC, Discogs, Spotify, YouTube (1 unit of the daily quota), the ACRCloud console, MusicBrainz, Eventbrite, Ticketmaster (only when switched on), Stripe, Shopify Buy Buttons, Shopify Admin, Resend, ElevenLabs, RoEx, StemSplit, Apple, Deezer, Odesli (at most once a minute) and Google News. Soundcharts (a paid call, counted in the budget on the team's share), Songstats (1 of 1,000 a month), Google Places, Geocoding, Time Zone and Routes (billed above the free cap; Routes is not enabled on the owner's project and reads as a known state, not a failure) and the R2 round trip (writes and deletes a test object) each have their own button, labelled with the cost, and are asked at most once a minute (min_interval_s 60): a second press inside it shows the stored answer instead of spending again.
- Routes: GET /admin/providers, POST /admin/providers/check, POST /admin/providers/check/<key>
- Files: provider_status.py; app.py admin_providers, admin_providers_check_all, admin_providers_check, _same_origin_post, _admin_never_framed (search "Providers (owner, 2026-09-23)"); templates/admin_providers.html; static/css/providers.css; soundcharts_budget.TEAM_PATHS; signal_providers.py MLCAdapter._call_raw/refusal, MusicBrainzAdapter._pace_lock; blob_store.py round_trip/_try_delete; linked from /admin/readiness, the Settings owner tools and the owner menu (_internal_tools)
- Access: Owner only via _owner_or_404: 404 for any other account, the sign-in redirect when signed out; a team seat is turned away by the team gate's /admin block before the route is reached, and _owner_or_404 refuses any act-on-behalf (team_as or acting_as); a reseller's staff can never act as an owner's account (current_user drops that act-on-behalf, and /resellers/<pid>/artists refuses an owner's email). A check press must come from the app's own page: a POST whose Origin or Referer names another host, or whose Sec-Fetch-Site is not same-origin, gets a 403 and asks nobody (the session cookie is SameSite=Lax, which a same-site page such as the Shopify storefront could otherwise ride). Every /admin/ response carries X-Frame-Options: DENY.
...
- OpenAI Images (gpt-image-1) - cover_ai.py render(), called from app.py:4172 /artwork/generate. Key REQUIRED (OPENAI_API_KEY); every render counted against the account's monthly covers. Unset, the Pollinations path below is used.
- Pollinations.ai image model - the no-key fallback: app.py:4172 /artwork/generate builds the image URL, app.py:4314 /artwork/save downloads it via music_apis.fetch_image_bytes(). NO key required; the browser loads the generation URL directly.
...
- OpenAI Images: no outbound request was made. The request and response shape were checked against OpenAI's published docs on 2026-09-23 and tests/test_cover_ai.py replaces cover_ai._transport in every test; the first real render happens when the owner sets OPENAI_API_KEY.
...
- Audit, 2026-09-23: the scanline overlay (.sbrk-door::after, a 16% black line every 3px over the whole glass, at rest and always) was a leftover of the first static design and broke the nothing-on-the-glass ruling; it is gone and tests/test_split_home.py bans `.sbrk-door::after` and any repeating-linear-gradient in the sheet. The leave glitches out only a screen that showed something: memberships-rack.js times each visit from mouseenter/focus and a pointer that only crossed a screen inside the cut-in's first step (LIT_AFTER_MS = 150, the first tenth of sbrk-assemble, where the reading is still at opacity 0 - a test reads it off the sheet) leaves the glass dark instead of flashing the whole tier. The owner's Settings radio reads "Rack: three dark screens, each shows its plan when you point at it", and the Split description no longer lists the stages rail. Tests also pin each screen's full aria-label (name, price, the plan's line, the coming-soon line) and the rack's own focus ring. split-home.css ?v=16, memberships-rack.js ?v=2.
