...
A persistent per-account to-do list with category, priority, due date, status and a list/board view.

- Because: create_action/set_action_status/list_actions all read and write the street_actions table scoped by user_id; probed 200 and templates/actions.html renders both the list and the board from those rows
- Routes: /actions (GET, POST)
- Files: app.py:5943 actions_page; command_center.py:43 create_action, :60 set_action_status, :72 list_actions, :85 open_actions; templates/actions.html; db.py:745 street_actions
- Access: plans.required_tier("/actions") = "artist" (in _ARTIST_PATHS); a fan plan gets 402 (verified). /actions is in team_areas.WHOLE_ACCOUNT, so a partial-room seat is bounced. Not owner-only.
...
Turns a Command Center alert or a preview page's reminder into a high-priority action, and confirms it on the page you were on.

- Because: the handler writes through cc.create_action into street_actions and redirects back to the referrer with ?action=<title>; an empty title is refused before any write (locked by tests/test_walk_command-tools_2026_09_20.py)
- Routes: /actions/from-alert (POST)
- Files: app.py:5979 action_from_alert; command_center.py:43 create_action; templates/command_center.html, templates/module_preview.html (the forms)
- Access: Any signed-in account; this path is not in _ARTIST_PATHS so there is no tier gate on the POST itself, and it is not in team_areas.WHOLE_ACCOUNT.
