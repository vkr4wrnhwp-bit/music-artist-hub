...
- Because: provider_status.py lists every outside service (PROVIDERS, in six FAMILIES: music data and rights; touring and places; money, shop and mail; storage, backup and monitoring; audio tools; public music lookups) with the variable names it needs, its switches, what it powers and the page it feeds, and one check that goes through the app's own adapter below any cache: sp.mlc_adapter().find_recordings by a known ISRC (sign-in alone is not proof), the Soundcharts adapter's _fetch_json through soundcharts_budget, SongstatsAdapter.probe, discogs_adapter()._send('/oauth/identity'), the Spotify client-credentials grant through spotify_provider._http, YouTubeAdapter._fetch_json('/channels', part='id') with the key redacted, acr_console._call GET /api/buckets, MusicBrainzAdapter.probe, Eventbrite's organizations through eventbrite_provider._http, a size-1 Discovery search through ticketmaster_provider._http (never while switched_off()), Places/Geocoding/Time Zone/Routes through venue_photos._http and venue_geo._http (never drive()), Stripe's webhook endpoints through stripe_provider._http_get (never setup_webhook_endpoint, which POSTs), shopify_buy.check(fresh=True) (never the Storefront mint), the Admin API's { shop { name } } through shopify_customers._post (never forget_grant), email_provider._api_get('/domains'), blob_store.round_trip, the ElevenLabs SDK's models.list with a 10-second limit and no retries, roex_client.health(), stemsplit_provider._call GET /balance, and the keyless music_apis lookups (Apple, Deezer, Odesli, Google News). A result is working (True), failing (False, in the vendor's own words) or not asked (None: not configured, off by an owner ruling or in a sandbox, or no safe check), and the lamp never reads "not asked" as failing. Nothing is called on a page view. Results are kept in app_kv under provider_status:<key>, with every configured value scrubbed first (as typed, URL-encoded and JSON-escaped, plus bearer/basic/key= shapes and a vendor's masked sk_ echo). Each check runs in its own thread under a hard time limit (12 seconds; The MLC 20, R2 45), and Check all runs the free set together, bounded by the slowest limit rather than the sum. Rows with no safe live check show what the app already keeps: the stored backup_last_run (failing when a run failed or is older than 26 hours), the newest beat_fingerprint_checks row for ACRCloud identify, the newest distribution application for Symphonic (every call files a real one), whether Sentry's SDK started in the worker, which inbound webhook secrets are held, and presence for Resend inbound and the suite sign-in secret. Bandsintown reads Dormant; the Chartmetric, SoundExchange and Spotify-metadata stubs read Not built and show no key. 234 tests in tests/test_provider_status.py, against a fake wire in urllib and the SDK client with a socket guard behind it.
- Rule: "Check all free services" runs only free, read-only checks that write nothing: The MLC, Discogs, Spotify, YouTube (1 unit of the daily quota), the ACRCloud console, MusicBrainz, Eventbrite, Ticketmaster (only when switched on), Stripe, Shopify Buy Buttons, Shopify Admin, Resend, ElevenLabs, RoEx, StemSplit, Apple, Deezer, Odesli (at most once a minute) and Google News. Soundcharts (a paid call, counted in the budget on the team's share), Songstats (1 of 1,000 a month), Google Places, Geocoding, Time Zone and Routes (billed above the free cap; Routes is not enabled on the owner's project and reads as a known state, not a failure) and the R2 round trip (writes and deletes a test object) each have their own button, labelled with the cost.
- Routes: GET /admin/providers, POST /admin/providers/check, POST /admin/providers/check/<key>
- Files: provider_status.py; app.py admin_providers, admin_providers_check_all, admin_providers_check (search "Providers (owner, 2026-09-23)"); templates/admin_providers.html; static/css/providers.css; soundcharts_budget.TEAM_PATHS; linked from /admin/readiness, the Settings owner tools and the owner menu (_internal_tools)
- Access: Owner only via _owner_or_404: 404 for any other account, the sign-in redirect when signed out; a team seat is turned away by the team gate's /admin block before the route is reached
